...
func getDailyMetric(c *gin.Context, field string, alias string) {
    ...
    // 🔹 Query ข้อมูล
    if field == "SleepHours" {
        // ใช้ < endUTC สำหรับ SleepHours ตามโค้ดเดิม
        err = db.Where("user_id = ? AND timestamp >= ? AND timestamp < ? AND sleep_hours <> ''",
            userID, startUTC, endUTC).
            Order("timestamp ASC").
            Find(&healthData).Error
    } else {
        // ใช้ <= endUTC สำหรับ Metric ปกติ เพื่อให้ครอบคลุมถึง 23:59:59
